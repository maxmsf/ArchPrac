...
if grep -q '<types>' ./changed-sources/package/package.xml ; then
  ...
  sfdx force:source:deploy -x "changed-sources/package/package.xml" --apiversion 59.0 --testlevel RunLocalTests --verbose -w 300 --checkonly -u $1 || exit 1
else
...
if grep -q '<types>' ./changed-sources/destructiveChanges/destructiveChanges.xml ; then
  ...
  sfdx force:mdapi:deploy -d "changed-sources/destructiveChanges" --ignorewarnings --verbose -w 300 --checkonly -u $1 --apiversion 59.0 || exit 1
else
